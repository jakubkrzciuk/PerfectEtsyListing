...
export const MarketingStudio: React.FC<MarketingStudioProps> = ({ mainImage, productName, price = "od 249 zł" }) => {
    ...
    return (
        <div className="bg-white rounded-[40px] p-10 border border-stone-200 shadow-xl space-y-10 animate-fade-in group/studio relative overflow-hidden">
            {/* Background Decor */}
            ...
            <div className="flex items-center justify-between relative z-10">
                <div>
                    ...
                    <p className="text-stone-400 text-xs mt-1">Stwórz profesjonalną kampanię w kilkanaście sekund</p>
                </div>
                <div className="flex gap-2">
                    <div className="flex flex-col items-end">
                        <span className="bg-stone-900 text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 relative z-10">
                ...
                <div className="xl:col-span-4 space-y-8 h-full">
                    ...
                    <div className="space-y-4">
                        <p className="text-[10px] font-black text-stone-500 uppercase tracking-[0.2em] flex items-center gap-2">
                            <Layout size={12} className="text-rose-500" /> 01. Format & Platforma
                        </p>
                        <div className="grid grid-cols-2 gap-2">
                            {TEMPLATES.map(t => (
                                <button
                                    ...
                                    className={`flex flex-col items-start p-4 rounded-2xl border-2 transition-all group ${selectedTemplate === t.id
                                            ? 'border-rose-500 bg-rose-50 text-rose-700 shadow-md'
                                            : 'border-stone-100 bg-stone-50 text-stone-400 hover:border-rose-200 hover:bg-white'
                                        }`}
                                >
                                    <div className="flex items-center gap-2 mb-2">
                                        {t.icon}
                                        <span className="text-[9px] font-black uppercase tracking-widest">{t.platform}</span>
                                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-stone-100">
                        <p className="text-[10px] font-black text-stone-500 uppercase tracking-[0.2em] flex items-center gap-2">
                            <Type size={12} className="text-rose-500" /> 02. Typografia i Tekst
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {FONTS.map(f => (
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-xl border text-[10px] font-bold transition-all ${selectedFont === f.id ? 'bg-stone-900 text-white border-stone-900' : 'bg-white text-stone-500 border-stone-200 hover:border-stone-400'
                                        }`}
                                >
                                    {f.name}
                                </button>
                        ...
                        <div className="space-y-3">
                            <div className="space-y-1">
                                <label className="text-[9px] font-black text-stone-400 uppercase">Główny Nagłówek</label>
                                <input
                                    type="text"
                                    value={headline}
                                    onChange={(e) => setHeadline(e.target.value)}
                                    className="w-full p-4 bg-stone-50 border border-stone-200 rounded-2xl text-xs font-bold focus:border-rose-400 outline-none transition-all placeholder:text-stone-300 shadow-inner"
                                    placeholder="Np. Minimal Heritage"
                                />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[9px] font-black text-stone-400 uppercase">Podtytuł / CTA</label>
                                <input
                                    type="text"
                                    value={subline}
                    ...
                    <div className="space-y-4 pt-4 border-t border-stone-100">
                        <p className="text-[10px] font-black text-stone-500 uppercase tracking-[0.2em] flex items-center gap-2">
                            <Palette size={12} className="text-rose-500" /> 03. Estetyka (Presety)
                        </p>
                        <div className="flex gap-2">
                            {FILTERS.map(f => (
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-xl border text-[9px] font-black uppercase transition-all ${selectedFilter === f.id ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-stone-400 border-stone-200 hover:border-amber-200'
                                        }`}
                                >
                                    {f.name}
                                </button>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label className="text-[9px] font-black text-stone-400 uppercase">Przyciemnienie Tła</label>
                                <span className="text-[9px] font-bold text-stone-900">{Math.round(overlayOpacity * 100)}%</span>
                            </div>
                            <input
                                type="range"
                                min="0"
                                ...
                                className="w-full accent-rose-500 h-1 bg-stone-100 rounded-full appearance-none cursor-pointer"
                            />
                        </div>
                    </div>

                    <button className="w-full py-5 bg-gradient-to-r from-stone-900 to-black text-white rounded-[20px] font-black text-xs uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 shadow-2xl shadow-stone-300 group">
                        <Download size={18} className="transition-transform group-hover:-translate-y-1" /> Eksportuj Grafiki 4K
                    </button>
                </div>

                {/* Right Canvas - Live Preview */}
                <div className="xl:col-span-8">
                    <div className="bg-stone-50 rounded-[40px] p-8 lg:p-16 border border-stone-200/50 flex items-center justify-center min-h-[600px] shadow-inner relative overflow-hidden group/canvas">
                        {/* Canvas Labels */}
